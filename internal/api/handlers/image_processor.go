...
// // ProcessImage downloads, compresses and uploads an image to S3.
...
// // uploadToS3 uploads the image bytes to S3 and returns the S3 URL.
...
// 	svc := s3.New(sess)
// 	encodeURL := url.QueryEscape(imageURL)
// 	// Generate a unique filename for the compressed image
// 	fileName := fmt.Sprintf("compressed-%s", encodeURL)
...
// 		return "",err
// 	}
//     s3URL := fmt.Sprintf("https://%s.s3.%s.amazonaws.com/%s", "golangbucket-yolo", awsRegion, fileName)
//     log.Printf("Successfully uploaded compressed image to S3: %s", s3URL)

//     return s3URL, nil
// }

// // StartImageProcessingWorker starts the image processing worker
